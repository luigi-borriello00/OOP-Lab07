...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
     ...
     */
	final Set<Sport> userSports = new HashSet<>();
    /**
    ...
    public Set<Sport> getIndividualSports(){ 
    	...
    	for(Sport x : this.userSports) {
    		if(x.isIndividualSport()) {
    			individualSport.add(x);
    		}
    	}
    ...
    public Set<Sport> getSportPracticedInPlace(Place p) { 
    	Set<Sport> inplaceSport = new HashSet<>();
    	for(Sport x : this.userSports) {
    		if(x.getPlace() == p) {
    			inplaceSport.add(x);
    		}
